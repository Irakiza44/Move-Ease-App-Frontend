...
export class UsersComponent implements OnInit {
  ...
  onSearch(): void {
    console.log('Search triggered with input:', this.searchInput);

    this.filteredContacts = this.contacts.filter(
      (contact) =>
        contact.role.toLowerCase().includes(this.searchInput.toLowerCase()) ||
        contact.email.toLowerCase().includes(this.searchInput.toLowerCase())
    );
